...
{
	u16_t http_resp_err = (u16_t)ATOI(argv[0], 0, "err");
...
{
	u16_t http_err = (u16_t)ATOI(argv[0], 0, "err");
...
{
    u8_t ctx_id = ATOI(argv[0], 0, "cti");
    u8_t ctx_state = ATOI(argv[1], 0, "cts");
    u8_t ctx_type = ATOI(argv[2], 0, "ctt");
...
static int bg95_sock_close(u8_t sock_id)
{
			   ...
			   socklen_t addrlen)
{
    ...
    snprintk(buf, sizeof(buf), "AT+QSSLOPEN=1,1,%u,\"%s\",%u,%u", sock->id,
		 modem_context_sprint_ip_addr(addr), dst_port, 0);
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
			     NULL, 0U, buf,
