 ...
 * ********************** SMBUS block ******************************
 * *****************************************************************/
static int smbus_block_read(struct bq40zxx_data *bq40zxx, u8_t reg_addr,
                    u8_t* rd_buf, size_t rd_sz)
{
...
static int smbus_block_write(struct bq40zxx_data *bq40zxx, u8_t reg_addr,
                    u8_t* wr_buf, size_t wr_sz)
{
...
{
	int status = 0;
    u8_t buf[MAC_WRITE_BLOCK_SZ(sizeof(subcommand))];
...
static int bq40zxx_read_data_block(struct bq40zxx_data *bq40zxx, u8_t offset,
...
static int bq40zxx_write_data_block(struct bq40zxx_data *bq40zxx, u8_t offset,
				   u8_t *data, u8_t bytes)
{
	u8_t rd_buf;
	int status = 0;
...
{
	int status;
    u8_t rd_buf[MAC_READ_BLOCK_SZ(2)];
    ...
    LOG_DBG("0x%x, 0x%x, 0x%x, 0x%x, 0x%x", rd_buf[0], rd_buf[1],
...
static int bq40zxx_print_fw_ver(struct bq40zxx_data *bq40zxx)
{
    ...
    //LOG_INF("0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", buf[0],
    //        buf[1], buf[9], buf[10], buf[11], buf[12]);
    LOG_INF("FW Ver:%04X, Build:%04X\n", (buf[2] << 8) | buf[3],
            (buf[4] << 8) | buf[5]);
    LOG_INF("Ztrack Ver:%04X\n", (buf[7] << 8) | buf[8]);
 ...
 * *****************************************************************/
static int bq40zxx_command_reg_read(struct bq40zxx_data *bq40zxx, u8_t command,
        u8_t *val, size_t val_sz)
{
	int status;
...
static int bq40zxx_command_reg_write(struct bq40zxx_data *bq40zxx, u8_t command,
				     u8_t data)
{
	u8_t rd_buf, reg_addr;
	int status = 0;
...
{
	struct bq40zxx_data *bq40zxx = dev->driver_data;
	float int_temp;
...
#define BQ40ZXX_INIT(index)                                                    \
	...
	static const struct bq40zxx_config bq40zxx_config_##index = {          \
		...
		.terminate_voltage = DT_INST_PROP(index, terminate_voltage),   \
									       ...
									       \
	DEVICE_AND_API_INIT(bq40zxx_##index, DT_INST_LABEL(index),             \
			    &bq40zxx_gauge_init, &bq40zxx_driver_##index,      \
