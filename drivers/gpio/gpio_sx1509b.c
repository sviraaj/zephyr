	...
	if (data_first) {
		...
		if (rc == 0) {
			rc = i2c_write(drv_data->i2c_master,
				       &outbuf.reg,
				       sizeof(outbuf) - sizeof(pins->data) - sizeof(pins->interrupt) -
				       sizeof(pins->interrupt_sense) - sizeof(pins->debounce_en),
				       cfg->i2c_slave_addr);
		}
	} else {
		rc = i2c_write(drv_data->i2c_master,
			       &outbuf.reg,
			       sizeof(outbuf) - sizeof(pins->interrupt_sense) -
			       sizeof(pins->interrupt) - sizeof(pins->debounce_en),
			       cfg->i2c_slave_addr);
	}
				   ...
				   enum gpio_int_trig trig)
{
	...
	if ((mode & GPIO_INT_ENABLE) && (drv_data->interrupt_sup == 0)) {
		return -ENOTSUP;
	}
