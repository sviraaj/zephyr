...
//  printk("Chip ID = 0x%04x%04x\n", ValH, ValL);
  ...
  wr8(REG_TOUCH_MODE + RAM_REG, 0x00);               // set touch on: continous - this is default
  ...
  wr8(REG_PWM_DUTY + RAM_REG, 0x20);                 // Backlight PWM duty (off)   
...
uint8_t Cmd_READ_REG_ID(void)
{
  ...
  SPI_Write(0x00);                  // REG_ID offset = 0x00
...
{ 
  ...
  uint32_t* data = (uint32_t*) k_calloc((length/4)+1, sizeof(uint32_t));
...
void Cmd_Text(uint16_t x, uint16_t y, uint16_t font, uint16_t options, const char* str)
{
  ...
  uint32_t* data = (uint32_t*) k_calloc((length / 4) + 1, sizeof(uint32_t)); // Allocate memory for the string expansion
...
void CoProWrCmdBuf(const uint8_t *buff, uint32_t count)
{
  ...
  do {                
    ...
    // Your buffer is printkBuf - limited to 64 bytes (or some other value, but always limited).
